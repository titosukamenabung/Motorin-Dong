...
public class LaporanTerlaris extends javax.swing.JPanel {
    ...
    private void loadDataTerlaris() {
        try {
            ...
            // Query: Gabung tabel -> Hitung Total Qty -> Filter Tanggal -> Urutkan Terlaris
            String sql = "SELECT m.merk, m.tipe, "
                       + "SUM(dt.jumlah) as total_qty, "
                       + "SUM(dt.subtotal) as total_uang " 
                       + "FROM detail_transaksi dt " 
                       + "JOIN transaksi t ON t.id_transaksi = dt.id_transaksi " 
                       + "JOIN motor m ON m.id_motor = dt.id_motor " 
            ...
            while (R.next()) {
                ...
                Object[] data = {no++, merk, tipe, qty, duitStr};
                model.addRow(data);
            }
