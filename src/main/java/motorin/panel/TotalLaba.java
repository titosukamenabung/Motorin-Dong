...
public class TotalLaba extends javax.swing.JPanel {
    ...
    private void tampilkanLaba() {
        ...
        try {
            // 3. QUERY SAKTI 3 TABEL (Transaksi + Detail + Motor)
            String sql = "SELECT t.tanggal, m.merk, m.tipe, m.harga_beli, d.jumlah, d.subtotal "
                       + "FROM transaksi t "
                       + "JOIN detail_transaksi d ON t.id_transaksi = d.id_transaksi "
                       + "JOIN motor m ON d.id_motor = m.id_motor "
            ...
            while (res.next()) {
                ...
                String barang  = res.getString("merk") + " " + res.getString("tipe");
                
                // KARENA DI 'detail_transaksi' ADA KOLOM 'jumlah', KITA PAKAI!
                int qty = res.getInt("jumlah"); 
                
                double modalSatuan = res.getDouble("harga_beli");
                double totalJual   = res.getDouble("subtotal"); // Harga Jual ke Customer

                // B. HITUNG CUAN
                double totalModal = modalSatuan * qty; // Modal dikali jumlah barang
                double keuntungan = totalJual - totalModal;

                // C. Masukkan ke Celengan
                totalSeluruhCuan += keuntungan;

                // D. Format Rupiah
                String vModal  = String.format("Rp %,.0f", modalSatuan);
                String vJual   = String.format("Rp %,.0f", totalJual);
                String vUntung = String.format("Rp %,.0f", keuntungan);
